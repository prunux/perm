...
sub info($){
    # my $self = shift;
    return "B Subkategorie - Berechtigungen",
           "Alle vergebenen und aktiven Berechtigungen fuer diese Subkategorie";}
...
sub template ($){
    ...
    #         name      desc                        widget
    return [
            [ 'sid',    'Subkategorie',               'hidcombo(combo=subcategory_combo,ref=subcategory)', '','\d+'  ],
#            [ 'start',  'Start Datum',               'text',                      '1900-01-01', '\d+-\d+-\d+' ],
#            [ 'end',    'End Datum',                 'text',                      '2100-12-31', '\d+-\d+-\d+' ],
            [ 'pers',   'Sortierung nach Person',     'checkbox',                  '',           '.*'          ],
...
--      AND auth_start >= ?
--      AND auth_end   <= ?
  ORDER BY $persord org_hid, element_name;
SQL
...
#      -param => [$p->{start},$p->{end}]
    ...
    $rep->group(  # title
        ...
        -head    => sub {
                        ...
                        my $out  = "Alle Berechtigungen ";
                           $out .= "vom $p->{start} bis $p->{end}\n";
                           $out .= '='x50 . "\n";
                           if ( $p->{pers})  {
                                $out .= "### Sortiert nach Mitarbeiter-Namen ###\n";
                           }
